...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, d_model))#torch.randn((1,1,self.d_model)).to(self.device)# TODO (learnable [CLS] token embedding) 1,1512
    ...
    def patchify(self, images):
        ...
        #try transpose too
        patches = patches.reshape(images.shape[0], -1, num_patches).permute(0,2,1).contiguous()#patches.permute(0,2,3,4,5,1).contiguous().view(N,-1,C*self.patch_dim*self.patch_dim)
        return patches
    ...
    def forward(self, images):
        ...
        # print(self.cls_token.shape,patches.shape) [1,1,512]
        # exit()
        #self.cls_token.expand(patches.shape[0],-1,-1)
        #torch.zeros((patches_embedded.shape[0], 1)
        output = torch.cat([self.cls_token.expand(patches.shape[0],-1,-1), patches_embedded], dim=1).to(self.device) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)
        # print(output.shape)
        # exit()
